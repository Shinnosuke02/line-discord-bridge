...
class ModernMessageBridge {
  constructor() {
    ...
    this.replyService = null; // 初期化後に設定
  ...
  async handleLineToDiscord(event) {
    try {
      ...
      if (discordMessage) {
        ...
        const cleanMessage = this.removeDisplayNameFromMessage(discordMessage, displayName, event.message.type);
        
        // 設定からWebhook機能の有効/無効を取得
        const useWebhook = config.webhook.enabled;
        
        const sentMessage = await this.sendToDiscord(mapping.discordChannelId, cleanMessage, {
        ...
        // メッセージマッピングを記録
        if (sentMessage && this.messageMappingManager) {
   ...
   * @param {string} messageType - メッセージタイプ（スタンプの場合は特別処理）
   * @returns {Object} クリーンなメッセージオブジェクト
   */
  removeDisplayNameFromMessage(message, displayName, messageType = null) {
  ...
  async processDiscordToLineMessage(messageData) {
    ...
    try {
      ...
      if (discordMessage.attachments && discordMessage.attachments.size > 0) {
        const results = await this.mediaService.processDiscordAttachments(Array.from(discordMessage.attachments.values()), lineUserId, this.lineService);
        // 最初の成功したメッセージIDを取得
      ...
      if (discordMessage.content && discordMessage.content.trim()) {
        ...
        if (urlResults.length === 0) {
          ...
          // LINEメッセージIDを取得（LINE APIの応答から）
          if (result && result.messageId) {
            lineMessageId = result.messageId;
          }
        } else if (urlResults.length > 0 && urlResults[0].lineMessageId) {
          lineMessageId = urlResults[0].lineMessageId;
        }
      }
      if (discordMessage.stickers && discordMessage.stickers.size > 0) {
        const results = await this.mediaService.processDiscordStickers(Array.from(discordMessage.stickers.values()), lineUserId, this.lineService);
        if (results.length > 0 && results[0].lineMessageId) {
          lineMessageId = results[0].lineMessageId;
        }
      }
      ...
      // メッセージマッピングを記録
      if (lineMessageId && this.messageMappingManager) {
